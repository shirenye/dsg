...
import com.dsg.gateway.entity.*;
import com.dsg.gateway.service.*;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
    ...
    public Mono<Void> filterBlackList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                String requestIp = DsgUtil.getServerHttpRequestIpAddress(request);
                ...
                if (forbid.get()) {
                    return DsgUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                            HttpStatus.NOT_ACCEPTABLE, new DsgResponse().message("黑名单限制，禁止访问"));
                }
            } else {
    ...
    public void saveRequestLogs(ServerWebExchange exchange) {
        ...
        if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
            ...
            ServerHttpRequest request = exchange.getRequest();
            String ipAddress = DsgUtil.getServerHttpRequestIpAddress(request);
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                return DsgUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                        HttpStatus.TOO_MANY_REQUESTS, new DsgResponse().message("访问频率超限，请稍后再试"));
            } else {
